...
本階段是一個**寫作→審核→修正→通過→下一段**的連續迴圈。**✍️ 首席主筆**嚴格遵循已定案的 `chapter_[X]_plan.md`，逐個節拍 (Beat by Beat) 撰寫初稿，**每完成一個 Beat 立即進入內部審核**，通過後才推進下一個 Beat。
...
*(AI 將啟動【階段一】，完整加載世界觀設定、背景設定與角色資料，輸出草案並等待您的修改)*

### ✍️ 指令二：確認大綱並開始寫作
**「大綱確認，開始執行第 [X] 章的撰寫與審核。」**
*(AI 將把大綱定稿寫入 `03_Outlines/chapter_[X]_plan.md`，並無縫推進【階段二】至【階段三】)*
